...
function ProductGalleryInfo({ id }) {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/product/find/${id}`).then((Response) => {
      setProduct(Response.data);
      setColorIds(Response.data.colorId);
    });
    axios
      .get(`http://localhost:8080/productImage/findByProductId/${id}`)
      .then((Response) => {
  ...
  const handleCount = (count) => {
    if (count <= 0) return alert('You have to choose at least one.');
  ...
  const handlePostWish = () => {
    if (colorId == 0) {
      ...
      axios
        .post('http://localhost:3005/wishLists', {
          productId: parseInt(id),
          quantity: quantitynumber,
          color: parseInt(colorId),
          size: size.current.value,
                          ...
                          <li className='nav-item'>
                            ...
                            >
                              <span className='thumb'>
                                <img
                                  className='img-fluid'
                                  src={`../../assets/images/products/${proMainImg1}`}
                                  alt={proMainImg1}
                                />
                              </span>
                            </button>
                          ...
                          <li className='nav-item' role='presentation'>
                            ...
                            >
                              <span className='thumb'>
                                <img
                                  className='img-fluid'
                                  src={`../../assets/images/products/${proMainImg2}`}
                                  alt={proMainImg2}
                                />
                              </span>
                            </button>
                          ...
                          <li className='nav-item' role='presentation'>
                            ...
                            >
                              <span className='thumb'>
                                <img
                                  className='img-fluid'
                                  src={`../../assets/images/products/${proMainImg3}`}
                                  alt={proMainImg3}
                                />
                              </span>
                            </button>
                  ...
                  <form onSubmit={addCart}>
                    <div className='product-content'>
                      ...
                      <span className='price'>
                        ${(100 - product.sale) * 0.01 * product.price}
                        {(100 - product.sale) * 0.01 * product.price ===
                        product.price ? (
                          ''
                        ) : (
